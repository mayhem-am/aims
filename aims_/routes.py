...
from aims_.models import User, Post, Tester
...
def register():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            if form.role.data.lower() == 'user':
                user = User(username=form.username.data, email=form.email.data, password=hashed_password)
            elif form.role.data.lower() == 'tester':
                user = Tester(username=form.username.data, email=form.email.data, password=hashed_password)
            db.session.add(user)
            db.session.commit()
...
def login():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            elif session['account_type'] == 'tester':
                user = Tester.query.filter_by(email=form.email.data).first()
                if user: 
                    if bcrypt.check_password_hash(user.password, form.password.data):
                        login_user(user, remember=form.remember.data)
                        next_page = request.args.get('next')
